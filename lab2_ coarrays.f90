program lab1
    ...
    ! инициализация coarrays
    integer :: my_proc, num_procs
    my_proc = this_image() ! id процесса локального процесса
    num_procs = num_images() ! количество процессов

    call cpu_time(start_time)

    ! генерация обучающей выборки (параллельная)
    call random_seed()
    
    do i = my_proc, elements_edu, num_procs
    ...
    ! генерация тестовой выборки (параллельная)
    do i = my_proc, elements_test, num_procs
        call random_number(f)
    ...
    do i = 1, elements_test
        do j = my_proc, elements_edu, num_procs      
        ...
        do j = my_proc, elements_edu - 1, num_procs
